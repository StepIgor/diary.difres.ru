...
function AddSubjectModal(props) {
    ...
    function addSubject(e) {
        ...
        if (name.replace(/ /g, '').length != 0 && JSON.parse(localStorage['subjects']).indexOf(name) == -1) {
            ...
            editErrorMsg('Поле пустое, либо предмет с таким названием уже существует')
        }
    }
                ...
                <Fade>
                    <div className="modal" onClick={preventHiding}>
                        <div className="modal-content">
                            <h4 className="blue-text">Добавить предмет</h4>
                            <input autoFocus={true} type="text" maxLength="64" placeholder="Введите название предмета" onChange={(e) => {
